...
def check_org_name_and_branch(lesson_name: str, org_name: str, gh_branch: str) -> Tuple[str, str]:
    ...
    if r.status_code != 200:
        ...
        try:
            ...
            raise Exception(
                f"Connection timeout when trying to find Southampton-RSG-Training/{lesson_name} repository"
            ) from exc
        ...
        try:
            ...
            raise Exception(
                "Connection timeout when trying to find {gh_branch} branch in {org_name}/{lesson_name}"
            ) from exc
        ...
        if r.status_code != 200:
            ...
            try:
                ...
                raise Exception(
                    "Connection timeout when trying to find gh-pages branch in {org_name}/{lesson}"
                ) from exc
            ...
            if r.status_code == 200:
                ...
                raise ValueError(
                    f"Branch '{gh_branch}' or 'gh-pages' does not exist in '{org_name}/{lesson_name}', or 'Southampton-RSG-Training'"
                )
...
for n, lesson_info in enumerate(website_config["lessons"]):
    ...
    os.system(
        f"git submodule add --force -b {gh_branch} https://github.com/{org_name}/{lesson_name}.git submodules/{lesson_name}"
    )
